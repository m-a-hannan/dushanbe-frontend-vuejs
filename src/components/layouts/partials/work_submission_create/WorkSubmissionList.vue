      ...
      >
        <div class="card-header" :id="'heading_' + work.id">
          <h2 class="mb-0">
            ...
            >
              <span>{{ index + 1 + ". " + work.bill.bill_name + "..." }}</span>
      ...
      axios
        ...
        .then(
          function (response) {
            ...
            // response.data has an array object that's why it doesn't show the data
...
.accordion .card-body .type-and-user .inner-box {
  padding: 10px 20px;
  background: #f5f5f5;
  border-radius: 5px;
}
.accordion .card-body small {
  opacity: 0.8;
}
